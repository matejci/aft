...
export default class MediaModal extends React.Component {
  ...
  handleCommentChange(event) {
    ...
    console.log("handleCommentChange root html: ")
    console.log(root.lastChild.text)

    var mentions = root.querySelectorAll('.mentions')
    console.log("mentions: ")
    console.log(mentions)
  }

  regex = (key, mentionValue, lastMention) => {
    // regex
    ...
    if (key == "@" || key == "#") {
      ...
      document.execCommand("insertHTML", false, "<span class='mentions' style='color:purple;'>"+ key +"</span>")
    ...
    } else if (!regex.test(lastMention) && lastMention.length > 1) {

      console.log("---")
      console.log("regex.test(lastMention)")
      console.log(regex.test(lastMention))
      console.log(this.state.lastMention)
      console.log("---")
      ...
      if (this.state.mentionStatus == true) {
        ...
        event.preventDefault()
        document.execCommand("insertHTML", false, "<span style='color:green;'>" + key + "</span>")
  ...
  trigger = (event) => {
    ...
    const root = parse(this.state.html)
    var mentions = root.querySelectorAll('.mentions')
    var lastMention = mentions[mentions.length-1]
    console.log("trigger mentions: ")
    console.log(lastMention)
    console.log(" ")
    ...
    // if (this.state.mentionStatus) {
      ...
      }, this.regex(key, this.state.mentionValue+key, lastMention.lastChild.text))
    } else if (key.length > 1) {
      // meta keys
      // ignore?
      // this.setState({
      //   mentionValue: ""
      // }, this.regex(key, "", lastMention))
    } else {
      // space key
      this.setState({
        mentionValue: ""
